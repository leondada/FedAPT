...
from PromptModels.clip_base import CustomCLIP_client,visual,CustomCLIP_temp
import clip
...
import logging
logging.basicConfig(
    filename=f'./logfinal/promptfl_{args.alpha}.log',
...
if args.data == 'domainnet' and args.alpha<0:
    ...
    for i,domain in enumerate(domains):
        traindata, test_data = get_domainnet_dloader(domain,args.batch_size,MultiViewDataInjector([precess,DEFAULT_AUG,DEFAULT_AUG]))#
        client_dataloaders.append(torch.utils.data.DataLoader(traindata,batch_size=args.batch_size, shuffle=True,num_workers=8, pin_memory=True))
...
models = []
models  = [CustomCLIP_client(out,clip_model,args.gctx,CSC=args.csc).cuda() for i in range(len(client_dataloaders))]
    ...
    if gf==0:#warmup
        for fe in range(10):
            ...
            for cl in this_round_clients:
                ...
                optimizer = make_optimizer(models[cl].prompt_learner,base_lr=args.learning_rate)#.AdamW
                for e in range(1):
                    # mmm=0
                    ...
                    if cl in [this_round_clients[0],]: 
                        top1,topk = evala(models[cl],visualmodel,client_testloaders[cl//6])
                        print(top1)

            weights = [1/len(this_round_clients)]*len(this_round_clients)
            prompt = [] 
            local_state  = models[0].state_dict()
            for k, cl in enumerate(this_round_clients):
                client_state = models[cl].state_dict()
            ...
            global_model.load_state_dict(local_state,strict=False)
            ...
            for cl,client in enumerate(tqdm(models)):
                ...
                for i, (images, label,indx) in enumerate((client_dataloaders[cl])):
                    with torch.no_grad():
                        ...
                        outputs1= client(output_norm,cls_only=True)
