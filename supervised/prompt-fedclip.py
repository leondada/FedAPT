...
from PromptModels.clip_base import CustomCLIP_client,CustomCLIP_ad
import clip
...
def get_parser():
    ...
    parser.add_argument('--data', default='domainnet',help='domainnet\office_caltech10')
...
import logging
logging.basicConfig(
    filename=f'./logfinal/fedclip.log',
...
for fe in range(50):
    ...
    if fe>0:
        for m,client in enumerate(models):
            client.prompt_learner.load_state_dict(global_model.ad_last.state_dict(),strict=False)

    
    for cl,client in enumerate(tqdm(models)):
        optimizer = make_optimizer(client.ad_last,base_lr=args.learning_rate)
        for e in range(1):
    ...
    for k, client in enumerate(models):
        client_state = client.ad_last.state_dict()
